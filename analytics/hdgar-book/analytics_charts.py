...
def plot_downloads():
	...
	QUERY = (
		'WITH days AS '
		'(SELECT protopayload_auditlog.requestMetadata.callerIp, DATE_TRUNC(timestamp, DAY) as day FROM `broad-datarepo-terra-prod-hca2.accesslogs.cloudaudit_googleapis_com_data_access_*`) ' 
		'SELECT day, COUNT(DISTINCT callerIp) as ipCount FROM days GROUP BY day ORDER BY day')

	df = client.query(QUERY).result().to_dataframe(create_bqstorage_client=False)
	df.set_index('day', inplace=True)

	ax = df.plot(title='Number of sessions per day downloading files', legend=False, xlabel='')
	plt.show()
