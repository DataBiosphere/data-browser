...
def authenticate(
    secret_name,
    first_service_params=ga_service_params,
    *other_service_params,
    ...
    flow = InstalledAppFlow.from_client_secrets_file(
        ANALYTICS_REPORTING_CLIENT_SECRET_PATH, scopes=all_scopes
    )
    ...
    built_systems = [
        build_service_system(service_params, credentials)
        for service_params in service_param_sets
    ]
...
def build_service_system(service_params, credentials):
    if len(service_params) == 4:
        ...
        service_name, service_version, param_subs_or_alt_api, query_func = (
            service_params[1:]
        )
...
def get_metrics_by_dimensions(
    metrics, dimensions, service_system=None, sort_results=None, **other_params
):
    ...
    if query_func is None:
        return param_subs_or_alt_api(
            service, metrics, dimensions, sort_results=sort_results, **other_params
        )

    return get_metrics_by_dimensions_v3_style(
        service,
        query_func,
        param_subs_or_alt_api,
        metrics,
        dimensions,
...
def get_metrics_by_dimensions_v3_style(
    service,
    query_func,
    param_subs,
    metrics,
...
def get_metrics_by_dimensions_v4_style(
    ...
    **other_params,
    ...
    params = {
        ...
        "metricFilter": parse_filter_expressions(
            [base_metric_filter, metric_filter], True
        ),
        "dimensionFilter": parse_filter_expressions(
            [base_dimension_filter, dimension_filter], False
        ),
        "orderBys": [
            (
                {"dimension": {"dimensionName": field}}
                if field in dimensions
                else {"metric": {"metricName": field}}
            )
            for field in sort_results
        ],
        "limit": max_results,
    }
    ...
    while has_more:
        result = (
            service.properties().runReport(property=property, body=params).execute()
        )
...
def v4_results_to_df(results, dimensions, metrics):
    ...
    for result in results:
        ...
        column_names = [
            header["name"] for header in result.get("dimensionHeaders", [])
        ] + [header["name"] for header in result.get("metricHeaders", [])]

        # Get data
        if "rows" in result:
            data = [
                [cell["value"] for cell in row.get("dimensionValues", [])]
                + [cell["value"] for cell in row.get("metricValues", [])]
...
def parse_filter_expression(text, is_metric):
    ...
    def parse_or(text):
        ...
        return (
            or_terms[0]
            if len(or_terms) == 1
            else {"orGroup": {"expressions": or_terms}}
        )
    ...
    return (
        and_terms[0]
        if len(and_terms) == 1
        else {"andGroup": {"expressions": and_terms}}
    )
    ...
    for filter in filters:
        ...
        if parsed:
            result = (
                parsed
                if result is None
                else {"andGroup": {"expressions": [result, parsed]}}
            )
    return result
