...
Until now, the way to browse datasets through the AnVIL portal has been to use the [AnVIL Dataset Catalog](https://anvilproject.org/data/consortia), which allows you to organize your search results based on the workspaces that contain a subset of a dataset by its consent code. The AnVIL Dataset Catalog provides summary level information of the dataset and study.

With the addition of the [AnVIL Data Explorer](https://prod.anvil.gi.ucsc.edu/explore/datasets), you’ll be able to sort the datasets you’re browsing based on the following managed access categories:
...
The [AnVIL Data Explorer](https://prod.anvil.gi.ucsc.edu/explore/datasets) can be found by navigating to the [AnVIL Portal](https://anvilproject.org/), and clicking on the Datasets button:
