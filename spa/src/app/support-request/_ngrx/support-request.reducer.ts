...
export function reducer(
    state: SupportRequestState = SupportRequestState.getDefaultState(),
    action: Action
): SupportRequestState {
    switch (action.type) {
        // Attachment rejected on drop of file
        case AttachmentDropRejectAction.ACTION_TYPE:
            return state.onAttachmentDropRejected(
                action as AttachmentDropRejectAction
            );

        // Support request form has been submitted
        case CreateSupportRequestRequestAction.ACTION_TYPE:
            return state.onCreateSupportRequestRequest(
        ...
        case CreateSupportRequestSuccessAction.ACTION_TYPE:
            return state.onCreateSupportRequestSuccess(
                action as CreateSupportRequestSuccessAction
            );

        // An error occurred during on submit of support request form
        case CreateSupportRequestErrorAction.ACTION_TYPE:
            return state.onCreateSupportRequestError(
                action as CreateSupportRequestErrorAction
            );
        ...
        case UploadAttachmentErrorAction.ACTION_TYPE:
            return state.onUploadAttachmentError(
                action as UploadAttachmentErrorAction
            );

        // Handle request to upload of attachment
        case UploadAttachmentRequestAction.ACTION_TYPE:
            return state.onUploadAttachmentRequest(
                action as UploadAttachmentRequestAction
            );

        // Handle successful upload of attachment
        case UploadAttachmentSuccessAction.ACTION_TYPE:
            return state.onUploadAttachmentSuccess(
                action as UploadAttachmentSuccessAction
            );
