...
export class ProjectMapper extends ProjectRowMapper {
    ...
    public mapRow(): EntityRow {
        ...
        const publications = this.mapPublications(
            this.row.projects,
        ...
        const accessionsByLabel = this.mapAccessions(
            this.row.projects[0].accessions
        );

        const entity = Object.assign({}, super.mapRow(), {
            accessionsByLabel,
            arrayExpressAccessions: getUnspecifiedIfNullValue(
                this.projects.arrayExpressAccessions
            ),
            contributors: contributors,
            deprecated:
                this.projectOverrides && this.projectOverrides.deprecated, // Check project edits to see if project has been deprecated
            duplicateOf:
                this.projectOverrides && this.projectOverrides.duplicateOf,
            fileType: (this.row.fileTypeSummaries || []).map(
                (fileType: FileTypeSummaryResponse) => fileType.format
            ),
            geoSeriesAccessions: getUnspecifiedIfNullValue(
                this.projects.geoSeriesAccessions
            ),
            insdcProjectAccessions: getUnspecifiedIfNullValue(
                this.projects.insdcProjectAccessions
            ),
            insdcStudyAccessions: getUnspecifiedIfNullValue(
                this.projects.insdcStudyAccessions
            ),
            projectDescription: getUnspecifiedIfNullValue(
                this.projects.projectDescription
            ),
            publications: publications,
            redirectUrl: this.projectOverrides
                ? this.projectOverrides.redirectUrl
                : null,
            supersededBy: this.projectOverrides
                ? this.projectOverrides.supersededBy
                : null,
            supplementaryLinks: this.rollupArray(
                this.row.projects,
                "supplementaryLinks"
            ),
        ...
        if (
            this.projectOverrides &&
            this.projectOverrides.contributorMatrices?.length
        ) {
            const mappedContributorMatrices = (entity as any)
                .contributorMatrices;
            const overrideContributedMatrices =
                this.projectOverrides.contributorMatrices;
            this.addContributorMatricesVisualizations(
                mappedContributorMatrices,
                overrideContributedMatrices
            );
        }
        ...
        if (
            !(entity as any).projectShortname &&
            this.projectOverrides.projectShortname
        ) {
            (entity as any).projectShortname =
                this.projectOverrides.projectShortname;
        }
    ...
    private addContributorMatricesVisualizations(
        contributorMatrices: ProjectMatrixView[],
        matrixOverrides: ProjectMatrixView[]
    ) {
        ...
        return accessionsResponse.reduce((accum, accessionResponse) => {
            ...
            accessionResponse.accession.split(";").forEach((id) => {
                ...
                accum.get(label).push({
                    id: trimmedId,
                    label,
                    url: this.accessionUrlPipe.transform(
                        trimmedId,
                        config.identifierOrgPrefix
                    ),
                });
            });
            return accum;
    ...
    private mapContributors(
        ...
        const projectContributors = this.rollupArray(
            this.row.projects,
            "contributors"
        );
        if (
