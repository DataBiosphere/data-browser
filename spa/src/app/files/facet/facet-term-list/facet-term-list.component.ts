...
export class FacetTermListComponent implements AfterViewInit {
    ...
    public onClickFacetTerm(fileFacetName: string, term: Term): void {
        ...
        // Update facet state
        this.facetTermSelected.emit(new FacetTermSelectedEvent(fileFacetName, term.name, !term.selected));
    }
    ...
    ngAfterViewInit() {
        ...
        for ( let termElRef of this.termElRefs.toArray() ) {
            ...
            const termHeight = termElRef.nativeElement.getBoundingClientRect().height;
            ...
            // term's or the previous term's visibility, to indicate there are more terms
                ...
                if ( percentVisible < 0.5 && lastVisibleTermElRef ) {
                    updatedParentHeight = 
                        previousHeightTally + this.calculateCheckBoxCutOffHeight(lastVisibleTermElRef.nativeElement);
                }
                // Otherwise more than half the current term is visible, cut current term height back to 50%
                else {
                    updatedParentHeight = heightTally + this.calculateCheckBoxCutOffHeight(termElRef.nativeElement);
                }
    ...
    private calculateCheckBoxCutOffHeight(termEl: HTMLElement): number {
        ...
        const height = (checkbox.offsetTop - termEl.offsetTop) + Math.floor(checkbox.getBoundingClientRect().height / 2);
        return height;
    }
}
