...
describe("SearchTermUrlService", () => {
    ...
    describe("parseQueryStringSearchTerms", () => {
        ...
        it("returns empty array when no filter is specified", () => {
            ...
            const searchTerms = searchTermUrlService.parseQueryStringSearchTerms({});
        ...
        it("parses valid filter", () => {
            ...
            const searchTerms = searchTermUrlService.parseQueryStringSearchTerms(params);
        ...
        it("parses valid age range filter", () => {
            ...
            const searchTerms = searchTermUrlService.parseQueryStringSearchTerms(params);
            const expected = [new QueryStringSearchTerm("foo", [{
                ageMin: 0,
                ageMax: 10,
                ageUnit: "year"
            }])];
            ...
            parseErrorParams.forEach(params => {
                ...
                it(`throws error for invalid filter: ${params.filter}`, () => {
                    ...
                    const expectedMessage = 
                        `${searchTermUrlService["ERROR_TEXT_PARSE"]} "${searchTermUrlService["PARAM_FILTER"]}": ${params.filter}`;
                    expect(() => searchTermUrlService.parseQueryStringSearchTerms(params)).toThrowError(expectedMessage);
                });
        ...
        describe("syntax errors", () => {
            ...
            syntaxErrorParams.forEach(params => {
                ...
                it(`throws error for invalid filter: ${params.filter}`, () => {
                    ...
                    const expectedMessage =
                        `${searchTermUrlService["ERROR_TEXT_SYNTAX"]} "${searchTermUrlService["PARAM_FILTER"]}": ${params.filter}`;
                    expect(() => searchTermUrlService.parseQueryStringSearchTerms(params)).toThrowError(expectedMessage);
                });
