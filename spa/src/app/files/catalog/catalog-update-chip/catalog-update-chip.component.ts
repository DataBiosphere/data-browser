...
export class CatalogUpdateChipComponent implements OnChanges {
    ...
    public getTooltipContent(): string {
        ...
        if ( this.isProjectNew() ) {
            return `New in the DCP2 ${this.runDate} update.`;
        }
        
        if ( this.isProjectUpdated() ) {
            return `Modified in the DCP2 ${this.runDate} update.`;
        }
    ...
    public ngOnChanges(changes: SimpleChanges) {
        ...
        if ( catalogUpdateUpdate ) {
            this.runDate = this.datePipe.transform(catalogUpdateUpdate.runDate, this.DATE_FORMAT, this.DATE_TZ);
        }
    }
}
