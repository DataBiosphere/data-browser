...
export class CatalogCanActivateGuard implements CanActivate, CanActivateChild {
        ...
        return combineLatest(
            ...
            switchMap(([catalogs, defaultCatalog]) => {
                ...
                if (this.isCatalogValid(catalogs, catalogParam)) {
                    this.store.dispatch(
                        new ErrorAction(`Catalog ${catalogParam} is invalid.`)
                    );
                    return of(false);
                }
     ...
     * @returns {string}
     */
    private isRemoveCatalogParam(
        defaultCatalog: Catalog,
        catalogParam: string
    ): boolean {
        return (
            catalogParam === DCPCatalog.DCP2 || catalogParam === defaultCatalog
        );
    }
}
