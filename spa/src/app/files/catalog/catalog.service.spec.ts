...
describe("CatalogService", () => {
    ...
    describe("Catalog request/response", () => {
        ...
        it("sets valid default catalog from config", (doneFn: DoneFn) => {
            ...
            spyOn(configService, "getDefaultCatalog").and.returnValue(
                CONFIG_DEFAULT_CATALOG
            );

            catalogService["bindCatalogsAPIResponse"](API_RESPONSE).subscribe(
                (atlas) => {
                    expect(atlas.defaultCatalog).toEqual(
                        CONFIG_DEFAULT_CATALOG
                    );
                    doneFn();
                }
        ...
        it("throws error on invalid default catalog in config", (doneFn: DoneFn) => {
            ...
            catalogService["bindCatalogsAPIResponse"](API_RESPONSE)
                ...
                .subscribe((errorMessage) => {
                    const expectedErrorMessage = `Invalid default catalog "${invalidDefaultCatalog}" for atlas "${CONFIG_ATLAS_NAME}".`;
        ...
        it("binds catalogs from response value", (doneFn: DoneFn) => {
            ...
            const expectedCatalogs = createExpectedBoundCatalogs(
                atlasName,
                API_RESPONSE
            );
            ...
            catalogService["bindCatalogsAPIResponse"](API_RESPONSE).subscribe(
                (atlas) => {
                    ...
                    expectedCatalogs.forEach((expectedCatalog) => {
                        expect(catalogs.indexOf(expectedCatalog)).not.toEqual(
                            -1
                        );
                    });
                    doneFn();
        ...
        it("throws error when no catalogs found for atlas", (doneFn: DoneFn) => {
            ...
            catalogService["bindCatalogsAPIResponse"](API_RESPONSE)
                .pipe(catchError((e) => of(e)))
                .subscribe((errorMessage) => {
                    expect(errorMessage as any).toEqual(
                        `No catalogs found for atlas "${dummyAtlasName}".`
                    );
                    doneFn();
                });
        });
        ...
        it("creates error model on HTTP error", (doneFn: DoneFn) => {
            catalogService["handleCatalogsAPIResponseError"]().subscribe(
                (error) => {
     ...
     * @returns {Catalog[]}
     */
    function createExpectedBoundCatalogs(
        atlasName: string,
        apiResponse: CatalogsAPIResponse,
