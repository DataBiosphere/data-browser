...
describe("ProjectRowMapper", () => {
    ...
    it("should create data source", () => {
        dataSource = new EntitiesDataSource<ProjectRowMapper>(
            of([PROJECT_ROW_SINGLE_VALUES]),
            ProjectRowMapper
        );
    ...
    it("should map library construction approach", (done: DoneFn) => {
        ...
        dataSource.connect().subscribe((rows) => {
            const mappedProject = rows[0];
            expect(mappedProject.libraryConstructionApproach).toEqual(
                projectToMap.protocols[0].libraryConstructionApproach[0]
            );
            done();
        });
    });
    ...
    it("should map project short name", (done: DoneFn) => {
        ...
        dataSource.connect().subscribe((rows) => {
            const mappedProject = rows[0];
            expect(mappedProject.projectShortname).toEqual(
                projectToMap.projects[0].projectShortname
            );
            done();
        });
    });
    ...
    it("should roll up and map short name values in a single project", (done: DoneFn) => {
        ...
        dataSource.connect().subscribe((rows) => {
            const mappedProject = rows[0];
            expect(mappedProject.projectShortname).toEqual(
                projectToMap.projects[0].projectShortname.join(", ")
            );
            done();
        });
    });
            ...
            fileTypes.forEach((fileType) => {
                ...
                it(`should map file type summary ${fileType} count`, (done: DoneFn) => {
                    ...
                    dataSource.connect().subscribe((rows) => {
                        ...
                        const fileTypeCount =
                            mappedProject.fileTypeCounts.get(fileType);
