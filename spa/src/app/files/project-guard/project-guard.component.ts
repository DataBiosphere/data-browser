...
export class ProjectGuardComponent implements OnInit {
    ...
    public isProjectIngestInProgress(projectIdToCheck: string): boolean {
        return (
            this.PROJECT_IDS_INGEST_IN_PROGRESS.indexOf(projectIdToCheck) >= 0
        );
    }
    ...
    ngOnInit() {
        ...
        this.activatedRoute.params
            .pipe(
                switchMap((params) =>
                    this.store.pipe(
                        select(selectProjectEditsById(params.id)),
                        take(1)
                    )
                ),
            ...
            .subscribe((projectEdits: Project) => {
                ...
                if (deprecated) {
                    const action = new ViewProjectDeprecatedAction(
                        projectId,
                ...
                if (withdrawn) {
                    const action = new ViewProjectWithdrawnAction(
                        projectId,
                        projectEdits.projectShortname,
                        this.window.location.href,
