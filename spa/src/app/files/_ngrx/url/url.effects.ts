...
export class UrlEffects {
    ...
    updateCatalogQueryParam$ = createEffect(
        () =>
            this.actions$.pipe(
    ...
    navigateToSelectedEntity$ = createEffect(
        () =>
            this.actions$.pipe(
    ...
    updateFilterQueryParam$ = createEffect(
        () =>
            this.actions$.pipe(
                ofType(
                ...
                filter(() => {
                    ...
                    // the get data flow
                    return (
                        this.urlService.isViewingEntities() ||
                        this.urlService.isViewingExport()
                    );
                }),
                switchMap(() =>
                    this.store.pipe(select(selectUrlSpecState), take(1))
                ),
                tap((urlSpecState) => {
                    const filterQueryString =
                        this.searchTermUrlService.stringifySearchTerms(
                            urlSpecState.selectedSearchTermsBySearchKey
                        );
