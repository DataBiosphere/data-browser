...
import {
    ...
    TableParams,
} from "../../table/pagination/table-params.model";
...
export class TableEffects {
    ...
    fetchNextPagedTableData$: Observable<Action> = createEffect(() =>
        this.actions$.pipe(
            ...
            switchMap(
                ([
                    ...
                    const index = this.convertSelectedEntityToTrackingIndex(
                    ...
                    // Fetch previous page.
                    const tableParams = (action as TableNextPageAction)
                        .tableParams;
                    return this.fetchEntitySearchResults(
                        catalog,
                        tableParams,
                        tableQueryParams
                    );
            ...
            map(
                (entitySearchResults: EntitySearchResults) =>
                    new TableNextPageSuccessAction(
                        entitySearchResults.tableModel
                    )
            )
        )
    );
    ...
    fetchSortedTableData$: Observable<Action> = createEffect(() =>
        this.actions$.pipe(
            ...
            switchMap(
                ([
                    ...
                    this.gtmService.trackEvent(
                        (action as FetchSortedTableDataRequestAction).asEvent({
                            catalog,
                            currentQuery: queryWhenActionTriggered,
                        })
                    );
                    ...
                    const tableParams = (
                        action as FetchSortedTableDataRequestAction
                    ).tableParams;
                    return this.fetchEntitySearchResults(
                        catalog,
                        tableParams,
            ...
            map(({ entitySearchResults, tableQueryParams }) => {
                const termCountsByFacetName =
                    tableQueryParams.tableState.tableModels.find(
                        (tableModel) => {
                            return (
                ...
                return new FetchTableDataSuccessAction(
                    tableModel.data,
    ...
    fetchPreviousPagedTableData$: Observable<Action> = createEffect(() =>
        this.actions$.pipe(
            ...
            switchMap(
                ([
                    ...
                    // Fetch previous page.
                    const tableParams = (action as TablePreviousPageAction)
                        .tableParams;
                    return this.fetchEntitySearchResults(
                        catalog,
                        tableParams,
            ...
            map(
                (entitySearchResults: EntitySearchResults) =>
                    new TablePreviousPageSuccessAction(
                        entitySearchResults.tableModel
                    )
            )
        )
    ...
    fetchTableData$: Observable<Action> = createEffect(() =>
        this.actions$.pipe(
            ...
            switchMap(([action, catalog, tableQueryParams]) => {
                ...
                let tableParams = Object.assign({}, DEFAULT_TABLE_PARAMS, {
    ...
    fetchTableModel$: Observable<Action> = createEffect(() =>
        this.actions$.pipe(
            ...
            map(
                (entitySearchResults: EntitySearchResults) =>
                    new FetchTableModelSuccessAction(
                        entitySearchResults.tableModel
                    )
            )
        )
        ...
        this.actions$.pipe(
            ...
            switchMap(
                ([
                    ...
                    // Send tracking event.
                    const event = (action as SelectProjectIdAction).asEvent({
                        catalog,
                        currentQuery: queryWhenActionTriggered,
                    });
                    this.gtmService.trackEvent(event);
     ...
     * @returns {GAIndex}
     */
    private convertSelectedEntityToTrackingIndex(
     ...
     */
    private fetchEntitySearchResults(
        catalog: Catalog,
        tableParams: TableParams,
        tableQueryParams
    ): Observable<EntitySearchResults> {
        const selectedSearchTermsBySearchKey =
            tableQueryParams.selectedSearchTermsBySearchKey;
