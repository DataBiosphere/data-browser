...
export class FileManifestEffects {
    ...
    fetchFilesFacets$: Observable<Action> = createEffect(() =>
        this.actions$.pipe(
            ...
            distinct(
                ...
                this.actions$.pipe(
                    ofType(
                        ClearFilesFacetsAction.ACTION_TYPE,
            ...
            map((entitySearchResults) => {
                return new FetchFilesFacetsSuccessAction(
    ...
    fetchFileTypeSummaries$: Observable<Action> = createEffect(() =>
        this.actions$.pipe(
            ofType(FetchFileManifestFileTypeSummariesRequestAction.ACTION_TYPE),
            concatMap(
                (action: FetchFileManifestFileTypeSummariesRequestAction) =>
                    of(action).pipe(
                        withLatestFrom(
                            this.store.pipe(select(selectCatalog), take(1)),
                            this.store.pipe(
            ...
            switchMap(([, catalog, selectedSearchTerms]) =>
                this.fileManifestService.fetchFileManifestFileSummary(
                    catalog,
                    selectedSearchTerms
                )
            ),
            map(
                (fileSummary: FileSummary) =>
                    new FetchFileManifestFileTypeSummariesSuccessAction(
                        fileSummary.fileTypeSummaries
                    )
            )
        )
    );
    ...
    fetchProjectFilesFacets$: Observable<Action> = createEffect(() =>
        this.actions$.pipe(
            ...
            switchMap(([, catalog, selectedSearchTerms]) => {
                const selectedSearchTermsBySearchKey =
                    selectedSearchTerms.reduce(
                        (accum, selectedSearchTerm: SearchTerm) => {
    ...
    fetchProjectSpeciesCount$: Observable<Action> = createEffect(() =>
        this.actions$.pipe(
            ...
            switchMap(([, catalog, selectedSearchTerms]) => {
                const selectedProjectId = selectedSearchTerms.find(
                    (searchTerm) =>
                ...
                >([
                    [
                        selectedProjectId.getSearchKey(),
            ...
            map((entitySearchResults) => {
                const speciesFacet = entitySearchResults.facets.find(
    ...
    fetchProjectSummary$: Observable<Action> = createEffect(() =>
        this.actions$.pipe(
            ...
            map(
                (fileSummary: FileSummary) =>
                    new FetchProjectFileSummarySuccessAction(fileSummary)
            )
        )
    );
    ...
    requestFileManifestUrl$: Observable<Action> = createEffect(() =>
        this.actions$.pipe(
            ofType(FetchFileManifestUrlRequestAction.ACTION_TYPE),
            concatMap((action: FetchFileManifestUrlRequestAction) =>
                of(action).pipe(
                    withLatestFrom(
                        this.store.pipe(select(selectCatalog), take(1)),
                        this.store.pipe(
            ...
            switchMap(
                ([action, catalog, searchTerms, fileFormatsFileFacet]) => {
                    ...
                    const { manifestFormat } =
                        action as FetchFileManifestUrlRequestAction;
