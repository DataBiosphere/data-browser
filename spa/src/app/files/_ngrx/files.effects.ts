...
export class FilesEffects {
    ...
    fetchFacetsAndSummary$: Observable<Action> = createEffect(() =>
        this.actions$.pipe(
            ofType(
    ...
    fetchFacets$: Observable<Action> = createEffect(() =>
        this.actions$.pipe(
            ...
            mergeMap(
                ({
                    ...
                    const emptyResultSet =
                        entitySearchResults.tableModel.data.length === 0;
                    if (emptyResultSet) {
                        const selectedEntity =
                            tableQueryParams.tableState.selectedEntity;
                    ...
                    const fetchSuccessAction = new FetchFacetsSuccessAction(
                        entitySearchResults.facets
                    );

                    // Set up search term action
                    const searchTermUpdatedAction =
                        new SearchTermsUpdatedAction(
                            entitySearchResults.searchTerms,
                            entitySearchResults.searchEntities
                        );
                    ...
                    if (
                        !(action as FetchFileFacetsRequestAction)
                            .updateTableData
                    ) {
                        return of(
                            fetchSuccessAction,
                            searchTermUpdatedAction,
                            new TermCountsUpdatedAction(
                                entitySearchResults.tableModel.termCountsByFacetName
                            )
                        );
                    }
                    ...
                    const selectedEntity =
                        tableQueryParams.tableState.selectedEntity;
                    const searchTermsBySearchKey =
                        tableQueryParams.selectedSearchTermsBySearchKey;
                    let tableDataAction;
                    if (
                        selectedEntity === EntityName.PROJECTS &&
                        this.isAnyProjectSelected(searchTermsBySearchKey)
                    ) {
                        tableDataAction = new FetchTableDataRequestAction(
                            entitySearchResults.tableModel.termCountsByFacetName
                        );
                    } else {
                        const tableModel = entitySearchResults.tableModel;
                        tableDataAction = new FetchTableModelSuccessAction(
                            tableModel
                        );
                    }
    ...
    fetchSelectedProjectsById$ = createEffect(() =>
        this.actions$.pipe(
            ofType(SetViewStateAction.ACTION_TYPE),
            concatMap((action) =>
                of(action).pipe(
                    withLatestFrom(
                        this.store.pipe(select(selectCatalog), take(1)),
                        this.store.pipe(
                            select(selectIsSelectedTermsLoading),
                            take(1)
                        ),
                        this.store.pipe(
            ...
            switchMap(
                ([action, catalog, loading, selectedProjectSearchTerms]) => {
                    return this.fileService.fetchEntitySearchResults(
                        catalog,
                        new Map([
                            [
                                FileFacetName.PROJECT_ID,
                                new Set(selectedProjectSearchTerms),
                            ],
                        ]),
            ...
            map((entitySearchResults) => {
                return new FetchSelectedProjectsSuccessAction(
                    entitySearchResults.searchEntities
                );
            })
        )
    ...
    fetchSummary$: Observable<Action> = createEffect(() =>
        this.actions$.pipe(
            ...
            map(
                (fileSummary: FileSummary) =>
                    new FetchFileSummarySuccessAction(fileSummary)
            )
        )
    );
