...
export class FacetState {
    ...
    constructor(
        ...
        this.fileFacets = this.facets.filter(
            (facet) => facet instanceof FileFacet
        ) as FileFacet[];
    }
    ...
    public clearAgeRange(action: ClearSelectedAgeRangeAction): FacetState {
        ...
        this.facetsByName.forEach((f) => {
            ...
            if (f.name === facetName) {
                fileFacetsByName.set(
                    f.name,
                    (f as FacetAgeRange).clearAgeRange()
                );
            } else {
        ...
        const selectedFacet = this.determineSelectedFacet(
            this.selectedFacet,
            newSelectedFacet
        );
    ...
    public setAgeRange(action: SelectFacetAgeRangeAction): FacetState {
        ...
        this.facetsByName.forEach((f) => {
            ...
            if (f.name === facetName) {
                fileFacetsByName.set(
                    f.name,
                    (f as FacetAgeRange).setAgeRange(action.ageRange)
                );
            } else {
                fileFacetsByName.set(f.name, f);
            }
     ...
     * @returns {FacetState}
     */
    public setSelectedFacetTermsFromViewState(
        ...
        const fileFacetNames = selectedFacetStates.map(
            (selectedFacetState: QueryStringSearchTerm) => {
                return selectedFacetState.facetName;
            }
        );

        // Create facets from query string search terms, and add to map
        const fileFacetsMap = selectedFacetStates.reduce(
            (accum, queryStringSearchTerm: QueryStringSearchTerm) => {
                const facetName = queryStringSearchTerm.facetName;
                if (
                    queryStringSearchTerm.facetName ===
                    FacetAgeRangeName.ORGANISM_AGE_RANGE
                ) {
                    accum.set(
                        facetName,
                        this.buildFacetAgeRangeFromQueryString(
                    ...
                    accum.set(
                        facetName,
                        this.buildFileFacetFromQueryString(
                            queryStringSearchTerm
                        )
                    );
                }
     ...
     * @returns {FileFacet}
     */
    private buildFileFacetFromQueryString(
        queryStringSearchTerm: QueryStringSearchTerm
    ): FileFacet {
    ...
    private static createFacetsMap(facets: Facet[]): Map<string, Facet> {
        return facets.reduce(
            (acc: Map<string, Facet>, facet: Facet): Map<string, Facet> => {
     ...
     */
    private determineSelectedFacet(
        currentSelectedFacet: Facet,
        newSelectedFacet: Facet
    ) {
        // We're working on the same selected facet
        if (
            this.selectedFacet &&
            newSelectedFacet.name === this.selectedFacet.name
        ) {
