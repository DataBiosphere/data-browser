...
export class ProjectEffects {
    ...
    fetchProject$: Observable<Action> = createEffect(() =>
        this.actions$.pipe(
            ofType(FetchProjectRequestAction.ACTION_TYPE),
            // Prevent dupe hits to fetch project. Reset distinct check on clear of project.
            distinct(
                (action: FetchProjectRequestAction) => action.projectId,
                this.actions$.pipe(
                    ofType(ClearSelectedProjectAction.ACTION_TYPE)
                )
            ),
            ...
            concatMap((action) =>
                of(action).pipe(
                    withLatestFrom(
                        ...
                        this.store.pipe(
                            select(selectProjectEditsById(action.projectId)),
                            take(1)
                        )
                    )
                )
    ...
    fetchProjectManifestFileLocation: Observable<Action> = createEffect(() =>
        this.actions$.pipe(
            ...
            mergeMap(([action, catalog]) => {
                ...
                const fileLocationUrl =
                    this.projectService.getProjectManifestFileLocationUrl(
                        catalog,
                        projectId,
                        projectTitle
                    );
                const fileLocation$ =
                    this.fileLocationService.fetchFileLocation(
                        fileLocationUrl,
                        killSwitch$
                    );
                return fileLocation$.pipe(withLatestFrom(of(action)));
            }),
            map(([fileLocation, action]) => {
                const { project } =
                    action as FetchProjectManifestFileLocationRequestAction;
                return new FetchProjectManifestFileLocationSuccessAction(
                    project.entryId,
                    fileLocation
                );
            })
    ...
    fetchProjectMatrixArchivePreview: Observable<Action> = createEffect(() =>
        this.actions$.pipe(
            ...
            distinct(
                (action: FetchProjectMatrixArchivePreviewRequestAction) =>
                    action.matrixId,
                this.actions$.pipe(
                    ofType(ClearProjectMatrixArchivePreviewAction.ACTION_TYPE)
                )
            ),
            concatMap((action) =>
                of(action).pipe(
            ...
            mergeMap(([action, catalog]) => {
                ...
                const { matrixId, matrixVersion } =
                    action as FetchProjectMatrixArchivePreviewRequestAction;
                const archiveFiles$ =
                    this.projectService.fetchProjectMatrixArchiveFiles(
                        matrixId,
                        matrixVersion
                    );
                return archiveFiles$.pipe(withLatestFrom(of(action)));
            }),
            map(([archiveFiles, action]) => {
                const { matrixId, project } =
                    action as FetchProjectMatrixArchivePreviewRequestAction;
                return new FetchProjectMatrixArchivePreviewSuccessAction(
                    project.entryId,
                    matrixId,
                    archiveFiles
                );
    ...
    fetchProjectMatrixFileLocation: Observable<Action> = createEffect(() =>
        this.actions$.pipe(
            ...
            map(([fileLocation, action]) => {
                const { fileUrl, project } =
                    action as FetchProjectMatrixFileLocationRequestAction;
                return new FetchProjectMatrixFileLocationSuccessAction(
                    project.entryId,
                    fileUrl,
                    fileLocation
                );
    ...
    trackProjectDownload$ = createEffect(
        () =>
            this.actions$.pipe(
                ofType(
                ...
                tap(([action, catalog, selectedSearchTerms]) => {
                    const terms = selectedSearchTerms
                        .map((searchTerm) => searchTerm.getSearchValue())
                        .join(", ");
                    this.gtmService.trackEvent(
    ...
    trackProjectAccessionClicked$ = createEffect(
        () =>
            this.actions$.pipe(
                ...
                tap(([action, catalog]) => {
                    this.gtmService.trackEvent(
                        (action as ViewProjectAccessionAction).asEvent({
                            catalog,
                        })
                    );
                })
    ...
    viewProjectDeprecated$ = createEffect(
        () =>
            this.actions$.pipe(
                ...
                tap(([action, catalog, queryWhenActionTriggered]) => {
                    this.gtmService.trackEvent(
                        (action as ViewProjectDeprecatedAction).asEvent({
                            catalog,
    ...
    viewProjectIntegration$ = createEffect(
        () =>
            this.actions$.pipe(
                ...
                tap(([action, catalog, queryWhenActionTriggered]) => {
                    this.gtmService.trackEvent(
                        (action as ViewProjectIntegrationAction).asEvent({
                            catalog,
                            currentQuery: queryWhenActionTriggered,
                        })
                    );
    ...
    viewProjectSupplementaryLink$ = createEffect(
        () =>
            this.actions$.pipe(
                ...
                tap(([action, catalog, queryWhenActionTriggered]) => {
                    this.gtmService.trackEvent(
                        (action as ViewProjectSupplementaryLinkAction).asEvent({
                            catalog,
                            currentQuery: queryWhenActionTriggered,
                        })
                    );
    ...
    viewProjectWithdrawn$ = createEffect(
        () =>
            this.actions$.pipe(
                ...
                tap(([action, catalog, queryWhenActionTriggered]) => {
                    this.gtmService.trackEvent(
                        (action as ViewProjectWithdrawnAction).asEvent({
                            catalog,
                            currentQuery: queryWhenActionTriggered,
                        })
                    );
