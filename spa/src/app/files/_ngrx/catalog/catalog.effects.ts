...
export class CatalogEffects {
        ...
        this.actions$.pipe(
            ...
            switchMap(() =>
                this.catalogService.fetchCatalogs().pipe(
                    map(
                        (atlas: Atlas) => new FetchCatalogsSuccessAction(atlas)
                    ),
                    catchError((errorMessage) =>
                        of(
                            new FetchCatalogsErrorAction(),
                            new ErrorAction(errorMessage)
                        )
                    )
                )
            )
            ...
            this.actions$.pipe(
                ...
                tap(([action, catalog]) => {
                    this.gtmService.trackEvent(
                        (action as ViewCatalogAction).asEvent({ catalog })
                    );
                })
            ),
        { dispatch: false }
    ...
    initCatalogUpdatedSinceLastVisit$: Observable<Action> = createEffect(() =>
        this.router.events.pipe(
            ...
            map(([, catalog]) => {
                const updatedSinceLastVisit =
                    this.catalogService.isCatalogUpdatedSinceLastVisit(catalog);
                return new SetCatalogUpdatedSinceLastVisitAction(
