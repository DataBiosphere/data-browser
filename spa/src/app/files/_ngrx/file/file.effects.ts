...
export class FileEffects {
    ...
    fetchFileFileLocation$: Observable<Action> = createEffect(() =>
        this.actions$.pipe(
            ...
            mergeMap(([action, catalog, queryWhenActionTriggered]) => {
                ...
                this.gtmService.trackEvent(
                    (action as FetchFileFileLocationRequestAction).asEvent({
                        catalog,
                ...
                const { fileUrl } =
                    action as FetchFileFileLocationRequestAction;
                ...
                return fileLocation$.pipe(withLatestFrom(of(fileUrl)));
            }),
            map(([fileLocation, fileUrl]) => {
                return new FetchFileFileLocationSuccessAction(
                    fileUrl,
                    fileLocation
                );
            })
