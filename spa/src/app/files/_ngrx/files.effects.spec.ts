...
import {
    ...
    FILES_STATE_WITH_SEARCH_TERM,
    PROJECTS_STATE_WITH_PROJECT_SEARCH_TERM,
    SAMPLES_STATE_WITH_SEARCH_TERM,
...
describe("FilesEffects", () => {
    ...
    beforeEach(() => {
        ...
        fetchEntitySearchResults = spyOn(
            filesService,
            "fetchEntitySearchResults"
        );
        fetchEntitySearchResults.and.returnValue(
            of(DEFAULT_PROJECTS_ENTITY_SEARCH_RESULTS)
        );
        spyOn(filesService, "fetchFileSummary").and.returnValue(
            of(FILE_SUMMARY)
        );
        ...
        describe("Projects", () => {
            ...
            it(`updates column term counts when "update table data" is set to false`, () => {
                ...
                const expected = cold("--(bcd)", {
                    b: new FetchFacetsSuccessAction(
                        DEFAULT_PROJECTS_ENTITY_SEARCH_RESULTS.facets
                    ),
                    c: new SearchTermsUpdatedAction([], []),
                    d: new TermCountsUpdatedAction(
            ...
            it("updates full table model when no project search term is selected", () => {
                ...
                const expected = cold("--(bcd)", {
                    ...
                    c: new SearchTermsUpdatedAction([], []),
                    d: new FetchTableModelSuccessAction(
            ...
            it("requests table data when a project search term is selected", () => {
                ...
                const expected = cold("--(bcd)", {
                    ...
                    c: new SearchTermsUpdatedAction([], []),
                    d: new FetchTableDataRequestAction(
                        DEFAULT_PROJECTS_ENTITY_SEARCH_RESULTS.tableModel.termCountsByFacetName
                    ),
                });
        ...
        describe("Search Exceptions", () => {
            ...
            beforeEach(async () => {
                ...
                store.overrideSelector(
                    ...
                    mockTableQueryParams
                );
                store.overrideSelector(
                    selectPreviousQuery,
                    mockSelectPreviousQuery
                );
