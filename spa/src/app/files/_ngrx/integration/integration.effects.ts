...
export class IntegrationEffects {
    ...
    fetchIntegrationsByProjectId: Observable<Action> = createEffect(() =>
        this.actions$.pipe(
            ...
            switchMap(([action, catalog, integrationsByProjectId]) => {
                const projectId = (
                    action as FetchIntegrationsByProjectIdRequestAction
                ).projectId;
                if (integrationsByProjectId.has(projectId)) {
                    return of({
                        projectId,
                        integrations: integrationsByProjectId.get(projectId),
                    });
                }

                return this.integrationService
                    .fetchIntegrationsByProjectId(catalog, projectId)
                    .pipe(map((integrations) => ({ projectId, integrations })));
            }),
            map(
                ({ projectId, integrations }) =>
                    new FetchIntegrationsByProjectIdSuccessAction(
                        projectId,
                        integrations
                    )
            )
