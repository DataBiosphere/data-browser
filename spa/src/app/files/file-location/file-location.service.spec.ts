...
describe("FileLocationService", () => {
    ...
    describe("Polling", () => {
        ...
        it("polls endpoint on response status 301", (doneFn: DoneFn) => {
            ...
            fileLocationService
                .fetchFileLocation(fileUrl, killSwitch$)
                .subscribe(() => {
                    expect(
                        fileLocationService["pollFileLocation"]
                    ).toHaveBeenCalled();
                    doneFn();
                });
        });
        ...
        it("polls endpoint until response status 302 is returned", (doneFn: DoneFn) => {
            ...
            const fileLocation$ = fileLocationService.fetchFileLocation(
                fileUrl,
                killSwitch$
            );
            const inProgress$ = fileLocation$.pipe(
                filter(
                    (fileLocation) =>
                        fileLocation.status === FileLocationStatus.IN_PROGRESS
                )
            );
            const completed$ = fileLocation$.pipe(
                filter(
                    (fileLocation) =>
                        fileLocation.status === FileLocationStatus.COMPLETED
                )
            );

            combineLatest(inProgress$, completed$).subscribe(
                ([inProgress, completed$]) => {
                    expect(inProgress.status).toEqual(
                        FileLocationStatus.IN_PROGRESS
                    );
                    expect(completed$.status).toEqual(
                        FileLocationStatus.COMPLETED
                    );
                    doneFn();
                }
            );
    ...
    describe("Binding API response", () => {
        ...
        it(`bind fileUrl from "Location" response value`, (doneFn: DoneFn) => {
            ...
            fileLocationService["bindFileLocationAPIResponse"](
                apiResponse
        ...
        it(`bind retryAfter from "Retry-After" response value`, (doneFn: DoneFn) => {
            ...
            fileLocationService["bindFileLocationAPIResponse"](
                ...
                expect(fileLocation.retryAfter).toEqual(
                    apiResponse["Retry-After"]
                );
                doneFn();
            });
        });
        ...
        it("translates 301 to IN_PROGRESS", () => {
            const status =
                fileLocationService["translateAPIResponseStatus"](301);
        ...
        it("translates 302 to COMPLETED", () => {
            const status =
                fileLocationService["translateAPIResponseStatus"](302);
        ...
        it("translates non 30x response status to FAILED", () => {
            const status =
                fileLocationService["translateAPIResponseStatus"](300);
        ...
        it("binds 301 response to in progress file location", (doneFn: DoneFn) => {
            ...
            fileLocationService
                ...
                .subscribe((fileLocation) => {
                    ...
                    killSwitch$.complete();
                    expect(fileLocation.status).toEqual(
                        FileLocationStatus.IN_PROGRESS
                    );
                    doneFn();
                });
    ...
    describe("buildFetchFileUrl", () => {
        ...
        it(`adds "/fetch" to "/path`, () => {
            ...
            const fetchFileUrl =
                fileLocationService["buildFetchFileUrl"](fileUrl);
            expect(fetchFileUrl).toEqual(
                `${ORIGIN}${APIEndpoints.FETCH}${PATH}${SEARCH}`
            );
        });
