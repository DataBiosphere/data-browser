...
export class ReleaseFilesModalComponent implements OnDestroy, OnInit {
     ...
     * @returns {string}
     */
    public getFileDownloadParagraph(): string {

        return "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Etiam at commodo diam, vel iaculis elit."
    }

    /**
     * Returns the display text for the specified release file type.
     *
     * @param {string} releaseFileType
     * @returns {string}
     */
    public getReleaseFileTypeDisplay(releaseFileType: string): string {

        return ReleaseFileType[releaseFileType];
    }

    /**
     * Create model of dataset to back modal. The release param is expected to have a single project containing a single
