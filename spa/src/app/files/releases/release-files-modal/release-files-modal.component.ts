...
export class ReleaseFilesModalComponent implements OnDestroy, OnInit {
    ...
    private descriptionByFileType = new Map<string, string>([
        [ReleaseFileType.ANNOTATED_CLUSTERS, "Seurat compatible output containing clustering and annotation information (Cumulus output)."],
        [ReleaseFileType.ANNOTATED_CLUSTERS, "File containing clustering and annotation information."],
        [ReleaseFileType.DIFFERENTIAL_EXPRESSION, "Differential expression with correction (Cumulus output)."],
        [ReleaseFileType.FILTER, "File describing cumulus filtering information."],
        [ReleaseFileType.CSV, "Zip of CSVs containing the differential expression analysis for use in R or Python packages."],
        [ReleaseFileType.PIPELINE_INPUT, "Loom file generated by the HCA Matrix service and used as input to the Cumulus pipeline."]
    ]);
    ...
    /**
     * @param {ReleaseFilesView} releaseFile
     * @returns {string}
     */
    public getFileDownloadParagraph(releaseFile: ReleaseFilesView): string {

        return this.descriptionByFileType.get(releaseFile.type);
    }
